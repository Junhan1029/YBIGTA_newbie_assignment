...
class MultiLayerPerceptron(object):
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        tanh_grad = (1 - z3**2)
        common_err = dz3 * tanh_grad 

        dh3 = common_err
        dW3 = np.dot(z2.T, dh3) + 2 * L2_norm * W3
        db3 = np.sum(dh3, axis=0)
        dz2 = np.dot(dh3, W3.T)

        leaky_grad = np.where(h2 > 0, 1, 0.01)
        dh2 = dz2 * leaky_grad
        
        dW2 = np.dot(z1.T, dh2) + 2 * L2_norm * W2
        db2 = np.sum(dh2, axis=0)
        dz1 = np.dot(dh2, W2.T)

        relu_grad = np.where(h1 > 0, 1, 0)
        dh1_main = dz1 * relu_grad
