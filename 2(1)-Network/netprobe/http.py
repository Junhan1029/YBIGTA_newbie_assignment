...
def parse_status_and_preview(raw: bytes, max_preview: int = 200) -> tuple[Optional[int], str, Optional[str]]:
    ...
    idx = raw.find(b"\r\n\r\n")
    if idx == -1:
        error = "Invalid HTTP response: missing header/body separator"
        return None, "", error
    ...
    try:
        ...
        return None, "", f"Header decode error: {e}"
    ...
    try:
        ...
        return None, "", "Invalid HTTP response: status code is not int"

    try:
        preview = body_bytes[:max_preview].decode("utf-8", errors="replace")
    except Exception as e:
        return status_code, "", f"Body decode error: {e}"
